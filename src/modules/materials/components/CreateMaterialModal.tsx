...
import { AcousticMaterial, THIRD_OCTAVE_BANDS, ThirdOctave, OCTAVE_BANDS, Octave } from '../types/AcousticMaterial';
...
const initialOctaveBands = OCTAVE_BANDS.reduce((acc, freq) => {
  acc[freq] = 0;
  return acc;
...
const getInitialFormData = (): FormData => ({
  ...
  octaveBands: initialOctaveBands,
                ...
                <FormField label="Bandas de Octava" tooltip="Valores de reducción acústica para cada banda de frecuencia.">
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {OCTAVE_BANDS.map(freq => (
                      <div key={freq} className="flex items-center">
                        <label className="w-20 text-sm font-medium text-gray-700">{freq} Hz</label>
                        <input type="number" value={formData.octaveBands[freq]} onChange={e => updateOctaveBandValue(freq, Number(e.target.value))} className="w-full p-2 border rounded" />
                      </div>
                    ))}
                  </div>
                </FormField>
