...
export const LayerTreePanel: React.FC<LayerTreePanelProps> = ({
  ...
  const expandedWalls = walls.map(
    (wall: any, idx: number) => wall.id || `wall-${idx}`
  );

  const [wallSwitches, setWallSwitches] = useState<Record<string, boolean>>(
    () => Object.fromEntries(expandedWalls.map((id) => [id, true]))
  );
  const [openingSwitches, setOpeningSwitches] = useState<
    Record<string, boolean>
  >(() => {
    const allOpenings = openings.map(
      (opening: any, oidx: number) => opening.id || `opening-${oidx}`
    );
  ...
  const [floorSwitches, setFloorSwitches] = useState<Record<string, boolean>>(
    () =>
      Object.fromEntries(
        floors.map((floor, idx) => [floor.id || `floor-${idx}`, true])
      )
  );
  const [ceilingSwitches, setCeilingSwitches] = useState<
    Record<string, boolean>
  >(() =>
    Object.fromEntries(
      ceilings.map((ceiling, idx) => [ceiling.id || `ceiling-${idx}`, true])
    )
  );
  ...
  const handleOpeningSwitch = (id: string, wallIndex: number) => {
    setOpeningSwitches((prev) => {
      ...
      const wallOpenings = openings.filter(
        (opening: any) => opening.wallIndex === wallIndex
      );
  ...
  function handleCeilingSwitch(id: string) {
  ...
  // Agrupa por planta/piso
  function groupByFloor(items: any[]) {
    return items.reduce((acc, item) => {
      const floor = item.level ?? item.floorNumber ?? "Planta 1";
  ...
  const groupedWalls = groupByFloor(walls);
  const groupedFloors = groupByFloor(floors);
  const groupedCeilings = groupByFloor(ceilings);
  ...
  return (
    <div className="flex flex-col h-full my-2">
      <div className="flex-1 overflow-y-auto pr-1">
        <Accordion type="multiple" defaultValue={Object.keys(groupedFloors)}>
          {Object.keys(groupedFloors).map((floorKey) => (
            <AccordionItem key={floorKey} value={floorKey}>
              <div className="flex items-center justify-between mb-2 px-2 rounded">
                {/* Flecha y texto juntos */}
                <AccordionTrigger className="flex flex-column-reverse items-center gap-2 font-bold text-[15px] text-black flex-none text-left px-0 py-0 bg-transparent shadow-none">
                  {floorKey}
                </AccordionTrigger>
                {/* Menú de tres puntos */}
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="p-1 rounded hover:bg-gray-200 transition ml-2">
              ...
              <AccordionContent>
                {/* Techo */}
                {groupedCeilings[floorKey]?.map((ceiling: any, idx: number) => {
                ...
                {/* Piso */}
                {groupedFloors[floorKey]?.map((floor: any, idx: number) => {
                ...
                <Accordion
                  type="multiple"
                  defaultValue={
                    groupedWalls[floorKey]?.map((wall: any, idx: number) => wall.id || `wall-${idx}`)
                  }
                >
                  {groupedWalls[floorKey]?.map((wall: any, idx: number) => {
                    const wallId = wall.id || `wall-${idx}`;
                    const wallOpenings = openings.filter(
                      (opening: any) => opening.wallIndex === idx
                    );
                    return (
                      <AccordionItem key={wallId} value={wallId}>
                        <AccordionTrigger className="flex items-center gap-2 px-2 py-1 bg-gray-50 hover:bg-gray-100 rounded transition-all min-h-[32px]">
                          <span className="flex items-center gap-2">
                            <Badge
                              variant="secondary"
                              className="mr-1 text-lg px-1 py-1 bg-white border border-gray-300 shadow-sm"
                            >
                            ...
                            <span className="text-[13px] font-medium text-black">
                              <span className="font-bold">
                                {wall.title} -{" "}
                                {wall.template?.descriptor || wall.descriptor}
                              </span>
                            </span>
                          </span>
                          <span className="text-[11px] text-gray-500 italic ml-2">
                          ...
                          <div className="ml-7 mt-1">
                            {wallOpenings.length > 0 ? (
                                  ...
                                  >
                                    ...
                                    <div className="flex items-center gap-1 ml-auto">
                                      <Switch
                                        ...
                                        onCheckedChange={() =>
                                          handleOpeningSwitch(openingId, idx)
                                        }
                                        className="scale-90"
                                      />
                                      <DropdownMenu>
                              ...
                              <div className="text-[11px] text-gray-400 ml-2">
                                Sin puertas ni ventanas en esta fachada.
                              </div>
                            )}
                          </div>
                        </AccordionContent>
